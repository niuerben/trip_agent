...
from .agent_logging import LoggingMCPTool
...
class AmapService:
    ...
    def plan_route(
        ...
        try:
            ...
            logger.info(f"路线规划结果: {result[:200]}...")
...
def _get_city_geocode_cached(
    ...
    try:
        ...
        logger.warning(f"⚠️ 高德城市中心解析跳过({city}): {type(error).__name__}: {error}")
        return None, None, None
...
# ---------------------------------------------------------------------------
# 图片服务（原 amap_photo_service.py）：
# 高德 Web 服务 REST 关键词搜索(extensions=all)获取 POI 官方图片。
...
class AmapPhotoService:
    ...
    def _fetch_pois(
        ...
        if not self.api_key:
            logger.warning("⚠️  高德 API Key 未配置, 无法获取图片")
            return []
        ...
        if str(data.get("status")) != "1":
            logger.warning(f"⚠️  高德返回非成功状态: {data.get('info')} (infocode={data.get('infocode')})")
            return []
    ...
    def search_pois(
        ...
        *,
        ...
        try:
            ...
            logger.warning(f"⚠️ POI 写入 Chroma 跳过: {type(error).__name__}: {error}")
        return pois
