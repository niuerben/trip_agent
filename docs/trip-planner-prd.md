...
行旅天下把一次旅行规划拆成两个阶段：先用结构化表单生成初代行程，再用上下文对话在同一份计划上做增量修改。系统全程维护一份结构化的计划 JSON，展示、修改、版本回退都以它为单一数据源。

核心行为：

1. 表单提交后生成初代行程，用户无需自己写提示词。
2. 后续修改走对话，指令转成计划上的操作集（增删改景点/餐厅/酒店、调序、控预算），无需重复填写目的地和日期。
3. 偏好（口味、酒店风格、预算等）以结构化约束传入模型，避免靠自由文本描述。
4. 地点、路线、天气经高德地图 POI/路线/天气接口获取，模型禁止凭记忆编造坐标。
5. 输出为每日时间线 + 地图标点 + 交通/餐饮/住宿/预算摘要，可保存、复用、分享。
...
| 保留用户数据 | 登录用户可保存历史对话、偏好和旅行计划，并继续修改 |
...
| 用户登录/注册 | 保存历史与偏好数据；兼容现有本地账号及 OAuth 方向 | P0 |
...
技术路线：结构化表单 → 结构化计划 JSON → 对话生成操作集修改计划 → 高德工具核验地点数据。计划以 JSON 为单一数据源，版本、预算、坐标均可校验、可比较、可回退，同时保留自然语言交互入口。

首个可交付闭环包含四件事：

1. 表单生成一份可查看的初代规划。
2. 口味、酒店风格、预算三类偏好进入结果，并附匹配理由。
3. 对话优化当前计划，生成可回退的新版本。
4. 打通 DeepSeek + HelloAgents + 高德地图工具做真实数据增强，失败可恢复。

闭环完成后再扩展收藏、分享、导出和预订能力。
