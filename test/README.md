...
| 真实 LLM 验收 | `test/integration/test_plan_agent_talk_real.py` | 否 | 显式设置 `RUN_REAL_SERVICE_TESTS=1` 后运行 |
...
python -m unittest test.unit.test_plan_agent_talk -v
...
- `test_plan_agent_talk.py`：PlanAgent 对话意图和 ChangeSet 解析
...
python -m unittest test.integration.test_plan_agent_talk_real -v
```
...
- `test/integration/test_plan_agent_talk_real.py`，作为显式真实 LLM 验收
